...
void Application::init()    // UI aspects setup
{
    ...
    welcomeText.setString("Word Suggestion");
    ...
    welcomeSubtext.setString("Word suggestion program using heapsort");
    welcomeSubtext.setPosition({260,440});
    welcomeSubtext.setFont(Font::getFont(BEBAS));
    welcomeSubtext.setCharacterSize(25);

    Helper::center(welcomeSubtext, welcomeText);
    Helper::bottom(welcomeSubtext, welcomeText, 20);

    emailText.setFillColor(sf::Color::Black);
    emailText.setString("Search for Word");
